...
/**
 * Creates resized images without exploding memory. Uses the method described in
 * android documentation concerning bitmap allocation, which is to subsample the
 * image to a smaller size, close to some expected size. This is required
 * because the android standard library is unable to create a reduced size image
 * from an image file using memory comparable to the final size (and loading a
 * full sized multi-megapixel picture for processing may exceed application
 * memory budget).
 */
...
public class UserPicture implements Serializable {
	...
	private boolean getStoredDimensions() throws IOException {
		 ...
		 * were possible to reliably wrap the input stream on a buffered stream,
		 * but it's not possible because decodeStream() places an upper read
		 * limit of 1024 bytes for a reset to be made (it calls mark(1024) on
		 * the stream).
		 */
		input.close();
