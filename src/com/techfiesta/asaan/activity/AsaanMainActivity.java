	...
	private void getProfileInformation() {
		try {
	        if (Plus.PeopleApi.getCurrentPerson(mGoogleApiClient) != null) {
	            ...
	            final String userId = currentPerson.getId();
	            Name userName = currentPerson.getName();
	            final String firstName = userName.getGivenName();
	            final String lastName = userName.getFamilyName();
    			            ...
    			            pUser.signUpInBackground(new SignUpCallback() {
    							...
    							public void done(ParseException e) {
    								if(e==null){
    									...
    									Log.d("FAILURE", "google+ sign up failed.so this user is already existing");
    									loginUser(personName,passWord);
    								}
...
//	            new LoadProfileImage(imgProfilePic).execute(personPhotoUrl);
	...
	*//**
	...
	public void onClickLogin(View v) {
		...
		View loginView = inflater.inflate(R.layout.login_dialog, null, false);
		final AlertDialog alert = new AlertDialog.Builder(AsaanMainActivity.this).create();
		alert.setView(loginView, 0, 0, 0, 0);
		alert.setCancelable(false);

		final EditText etUserName = (EditText) loginView.findViewById(R.id.etUserName);
		final EditText etPassword = (EditText) loginView.findViewById(R.id.etPassword);
	...
	public void loginUser(String username,String password){
		ParseUser.logInInBackground(username, password, new LogInCallback() {
			...
			public void done(ParseUser user, ParseException e) {
				if (user != null) {
					Log.d("SUCCESS", user + " is logged in");
					isLogin=true;
					setLoginVisibility(isLogin);
				} else {
