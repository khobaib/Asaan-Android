...
public class PaymentInfoActivity  extends Activity{
	...
	private void  saveTokenInGAE(Token token)
	{
	   ...
	   // cardObj.addProperty("currency", card.get);
	    //cardObj.addProperty("default",);
	    cardObj.addProperty("exp_month", card.getExpMonth());
	    cardObj.addProperty("exp_year", card.getExpYear());
	    //cardObj.addProperty("fundingType", );
	    cardObj.addProperty("id", card.getFingerprint());
	    cardObj.addProperty("last4", card.getLast4());
	    //cardObj.addProperty("modifiedDate",);
	    cardObj.addProperty("name", card.getName());
	    //cardObj.addProperty("provider",);
	   // cardObj.addProperty("providerCustomerId",);
	   // cardObj.addProperty("refreshToken",);
	    cardObj.addProperty("state", card.getAddressState());
	   // cardObj.addProperty("userId",);
		...
		private void updateMonth_YearSpinners() {
			...
			ArrayAdapter<String> sMonthAdapter = new ArrayAdapter<String>(PaymentInfoActivity.this, R.layout.spinner_textview,
					monthList);
			...
			ArrayAdapter<String> sYearAdapter = new ArrayAdapter<String>(PaymentInfoActivity.this, R.layout.spinner_textview, yearList);
		...
		private class PostCardInfo extends AsyncTask<Void,Void,Void>
		{
			...
			try {
				saveUserCard = AsaanMainActivity.mUserendpoint.saveUserCard(userCard);
